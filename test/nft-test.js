...
describe("NFT", async function () {
  it("should be able to mint, transferFrom, burn. And it should return appropriate name, symbol, totalSupply, tokenURI, ownerOf, balanceOf", async function () {
    ...
    console.log(badSigner.address)
    nft.transferOwnership(badSigner.address);
    //await nft.connect(badSigner).transferOwnership(signer.address);
    //nft.connect(badSigner).mint(signer.address);
    console.log(`nft deploy tx hash: ${nft.deployTransaction.hash}`);
    console.log(`greeter contract address: ${nft.address}`);

    // mint tokenId = 0
    const mint0Tx = await nft.mint(signer.address);
    await mint0Tx.wait();
    ...
    // expect(await nft.tokenURI(0)).to.equal("https://hogehoge/api/v1/tokens/0")
    ...
    // expect(await nft.tokenURI(1)).to.equal("https://hogehoge/api/v1/tokens/1")
    ...
    // expect(await nft.tokenURI(2)).to.equal("https://hogehoge/api/v1/tokens/2");
    ...
    nft.connect(signer).mint(signer.address);
  });
});
